...
export default function LivePreview({ files = {} }) {
  ...
  const liveCode = useMemo(() => {
    ...
    const appCandidates = ['/src/App.jsx', '/src/App.js', '/src/App.tsx', '/src/App.ts'];
    ...
    return (
      <div className="h-full w-full flex flex-col items-center justify-center p-6 text-sm">
        ...
        <ul className="mb-3 text-xs list-disc list-inside text-neutral-200">
          {commonJsFiles.map((f) => (
            <li key={f}><code className="px-1">{f}</code></li>
          ))}
        </ul>
        {files['/index.html'] && files['/index.html'].code ? (
          <div className="mt-4">
            <div className="mb-2 text-xs text-neutral-300">This project contains a static <code>/index.html</code>. You can preview it below:</div>
            <iframe
        ...
        <div className="mt-4 text-xs text-neutral-400 max-w-xl">
          The in-browser editor preview only supports browser-ready ESM code (import/export). Convert CommonJS-style <code>require()</code> calls and <code>module.exports</code> to ESM imports/exports or run the project in a bundler/server for a full preview.
        </div>
      </div>
    );
  }
  ...
  if (!liveCode) {
    ...
    return (<div className="h-full flex items-center justify-center text-slate-400">No runnable JS files found to preview.</div>);
  }
